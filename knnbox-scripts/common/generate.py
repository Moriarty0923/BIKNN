...
def _main(args, override_args, output_file):
    ...
    if override_args is not None:
        ...
        ###########  knn code 只更新knn相关args    ###################
        for key,value in vars(override_args).items():
            if key in ["arch","knn_mode","knn_temperature","knn_lambda","knn_k","knn_datastore_path","knn_inference_mode","mask_mode","knn_max_k","knn_combiner_path"]:
    ...
    # import pdb;pdb.set_trace()
    # 不加没法加载训练的metak的参数
    if args.knn_mode=="inference" and args.knn_combiner_path!=None:
        models[0].decoder.combiner = RobustCombiner.load(args.knn_combiner_path)
