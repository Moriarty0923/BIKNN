...
def main(args):
    ...
    train_meter.stop()
    # ## knnbox related code start >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # if "new_datastore" in global_vars() and args.renew_datastore !="no_renew" :
    ...
    #     good_mask = corr_times>=wrong_times
    ...
    for i, samples in enumerate(progress):
        ...
        if log_output is not None:  # not OOM, overflow, ...
            ...
            if num_updates % args.log_interval == 0:
                ...
                #     good_mask = corr_times>=wrong_times
                #     # import pdb;pdb.set_trace()
                #     # good_keys = torch.tensor(old_datastore['keys'].data[good_mask.unsqueeze(-1).expand(old_datastore['keys'].data.shape).cpu()])
                #     # good_vals = torch.tensor(old_datastore['vals'].data[good_mask.cpu()])
                #     logger.info("bad_vals_numbers: {} ".format((~good_mask).sum()))
                #     logger.info("new_vals_numbers: {} ".format(new_datastore['vals'].data.nonzero()[-1][-1]))
                #     # print("bad_vals_numbers: " , (~good_mask).sum())
                #     # print("new_vals_numbers: ", new_datastore['vals'].data.nonzero()[-1][-1])
                #     # good_mask = corr_times>=wrong_times
