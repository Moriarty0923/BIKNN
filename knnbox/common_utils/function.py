...
########   for nar knn #############
def filter_mask_tokens(prev_output_token,target,unk_idx=3):
    mask = prev_output_token.eq(unk_idx)
    masked_tokens = target.masked_select(mask)
...
def select_correct_position_mask(features,tokens, tgt_tokens,token_mask,pad_idx=1):
    r"""
    use the mask to chose keys 
    ...
    Return: (*, dim)
    """
    mask = (tokens == tgt_tokens) & token_mask
    keys = select_keys_with_pad_mask(features,mask)
    return mask, keys

def select_wrong_position_mask(features,tokens, tgt_tokens,token_mask,pad_idx=1):
    r"""
    use the mask to chose keys 
...
def select_all_position_mask(features,tokens, mask,pad_idx=1):
    r"""
    use the mask to chose keys 
