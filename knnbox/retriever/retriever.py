...
class Retriever:
    ...
    def retrieve(self, query, return_list = ["vals", "distances"], k = None , step=None ):
        r""" 
        ...
        if not hasattr(self.datastore, "faiss_index") or self.datastore.faiss_index is None or ("keys" not in self.datastore.faiss_index and "keys_0" not in self.datastore.faiss_index):
        ...
        for data_name in return_list:
            if data_name not in ["distances", "indices", "k", "query"]:
                ...
                if "vals_" in data_name:
                    ret["vals"] = torch.tensor(self.datastore[data_name].data[indices], device=query.device)
                else:
