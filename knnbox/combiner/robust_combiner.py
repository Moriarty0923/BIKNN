...
class MetaKNetwork(nn.Module):
    ...
    def forward(
        ...
        noise_logit = self.distance_fc1(all_key_feature).squeeze(-1)  # 形状 (B, S, K)
        # noise_logit = self.distance_fc1(all_key_feature).squeeze(-1)  # 形状 (B, S, K)
        
        # 使用 alpha 调整 sim_lambda
        sim_lambda = self.distance_func(torch.cat([top_prob.log(), knn_key_feature.log(), network_select_probs.log()], -1))
        # sim_lambda = sim_lambda_unweighted * alpha.unsqueeze(-1).unsqueeze(-1)  # 形状 (B, S, 1)
