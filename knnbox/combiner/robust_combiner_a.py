...
class MetaKNetwork(nn.Module):
    def __init__(
        ...
        super(MetaKNetwork, self).__init__()
        self.max_k = max_k    
        self.device = device
        self.midsize = midsize
        self.midsize_dc = midsize_dc
        ...
        self.distance_func = nn.Sequential(
            nn.Linear(self.max_k * 2 + self.topk_wp, 1),
        )
        self.distance_fc1 = nn.Sequential(
            nn.Linear(2 + 1, self.midsize_dc),  # 加1是因为alpha的维度
            nn.Tanh(),
            nn.Linear(self.midsize_dc, 1),
        )
    ...
    def forward(
        ...
        label_counts = self._get_label_count_segment(tgt_index)
        all_key_feature = torch.cat([knn_key_feature.log(), network_select_probs.log()], -1)
        top_prob, top_idx = torch.topk(network_probs, self.topk_wp)
        knn_feat = torch.cat([knn_dists, label_counts.float()], -1) 

        # 将 alpha 扩展并与 all_key_feature 融合
        alpha_expanded = alpha.unsqueeze(-1).unsqueeze(-1).expand(-1, S, K)
        all_key_feature_with_alpha = torch.cat((all_key_feature, alpha_expanded), dim=-1)
        
        # 使用融合后的特征计算 noise_logit
        noise_logit = self.distance_fc1(all_key_feature_with_alpha).squeeze(-1)
        ...
        knn_lambda = F.softmax(torch.cat([lambda_logit[:, :, :1], sim_lambda], -1), -1)[:, :, :1]
        tempe = torch.sigmoid(lambda_logit[:, :, 1:2])
        probs = F.softmax(-knn_dists * tempe + noise_logit, -1)
